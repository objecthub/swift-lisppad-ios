...
;;;
;;; This is a demo showing how to load photos, create photos, display them
;;; in the console and save them in the iOS photo library. When executed,
;;; this demo will load a photo of a landscape and a logo image. It will
;;; create a new photo in which the logo is used as a watermark. In
;;; addition, a copyright message is added.
;;;
...
(define (scale-to-fit s max)
  (cond ((> (size-width s) (size-width max))
          (scale-to-fit
            (size (size-width max)
                  (* (size-height s)
                     (/ (size-width max) (size-width s)))) max))
        ((> (size-height s) (size-height max))
          (scale-to-fit
            (size (* (size-width s)
                     (/ (size-height max) (size-height s)))
                  (size-height max)) max))
        (else s)))

;; Scale an image `image` to fit the given boundaries `max` and place
;; it in the middle of rectangle `frame`, drawn in color `col`.
(define (scale-image image max frame col)
  (let* ((bounds (scale-to-fit (image-size image) max))
         (center (move-point
                   (opt rect-point frame zero-point)
...
;; Create a new drawing combining the photo and the logo and including
;; a copyright message at the bottom right corner.
(define composed-drawing
  (let* ((photo-size      (image-size photo))
         ...
         (copyright-point (point (- (size-width photo-size)
                                    (size-width copyright-size) 120)
                                 (- (size-height photo-size) 200))))
    (drawing
      (draw-image photo (rect zero-point photo-size))
      (draw-image logo (rect (point 120 120)
                             (scale-size (image-size logo) 0.75))
                       1.0 'source-over)
      (draw-text copyright-text copyright-point copyright-font white)
      (draw-text copyright-text (move-point copyright-point -6 -6)
                 copyright-font black))))

...
_area_ is a region of interest provided as a rectangular of form `((x . y) . (width . height))` in normalized coordinates relative to the image’s lower-left corner; default is `((0 . 0) . (1 . 1))` (which is equivalent to `#f`). _num_ is the maximum number of rectangular shapes to detect; default is `#f` which will not constrain the number of results.

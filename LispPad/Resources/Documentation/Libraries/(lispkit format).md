...
**(format _[port] [config] [locale] [tabw [linew]] [aligndisp] cntrl arg ..._)** &nbsp;&nbsp;&nbsp; <span style="float:right;text-align:rigth;">[procedure]</span>  
...
Formatting configuration _config_ defines environment variables influencing the output of some formatting directives. If _config_ is not provided, the formatting configuration from parameter object `current-format-config` is used. For convenience, some environment variables, such as _locale_, can be overridden if they are provided when `format` is being invoked. _locale_ is a symbol and refers to a locale identifier like `en_US` that is used by locale-specific formatting directives. _tabw_ defines the maximum number of space characters that correspond to a single tab character. _linew_ specifies the number of characters per line; this is used by the justification directive only. _aligndisp_ is a boolean overriding the "align by display width" setting (see `format-config-align-display`); when set to `#t`, padding and justification directives measure text using its terminal display width (accounting for wide/double-width characters) instead of the number of characters.
...
**(format-config _[parent] [locale] [tabw [linew]] [aligndisp]  (tag cntrl [config]) ..._)** &nbsp;&nbsp;&nbsp; <span style="float:right;text-align:rigth;">[procedure]</span>  

Creates a new formatting configuration with _parent_ as parent configuration. If _parent_ is not provided explicitly, `current-format-config` is used. If _parent_ is `#f`, the new formatting configuation will not have a parent configuration. _locale_ refers to a locale identifier like `en_US` that is used by locale-specific formatting directives. _tabw_ defines the maximum number of space characters that correspond to a single tab character. _linew_ specifies the maximum number of characters per line. _aligndisp_ is a boolean defining whether padding and justification directives should measure text using its terminal display width instead of its character count (see `format-config-align-display`).
...
**(make-format-config _parent locale tabw linew aligndisp_)**  
...
_locale_ refers to a locale identifier like `en_US` that is used by locale-specific formatting directives. _tabw_ defines the maximum number of space characters that correspond to a single tab character. _linew_ specifies the maximum number of characters per line. _aligndisp_ is a boolean defining whether padding and justification directives should measure text using its terminal display width instead of its character count (see `format-config-align-display`).
...
**(format-config-align-display)** &nbsp;&nbsp;&nbsp; <span style="float:right;text-align:rigth;">[procedure]</span>  
**(format-config-align-display _config_)**  

Returns a boolean defining whether padding and justification directives (e.g. `~<n>A`, `~<n>@A`) of format configuration _config_ measure text using its terminal display width instead of its plain character count. Display-width-based alignment correctly accounts for characters that occupy more than one column on a terminal, such as East Asian wide characters and many emoji. If _config_ defines this setting itself, it is being returned. Otherwise, the setting of the parent configuration of _config_ gets returned. If _config_ is not provided, the default configuration `current-format-config` is used. The default is `#f`, i.e. alignment is based on the number of characters.

**(format-config-align-display-set! _align?_)** &nbsp;&nbsp;&nbsp; <span style="float:right;text-align:rigth;">[procedure]</span>  
**(format-config-align-display-set! _config align?_)**  

Sets whether padding and justification directives of format configuration _config_ should measure text using its terminal display width to _align?_. If _align?_ is `'()`, the setting gets removed from _config_ (but might still get inherited from _config_'s parents); note that, unlike the other `format-config-*-set!` procedures, `#f` sets an explicit "align by character count" override rather than removing the setting. If _config_ is not provided, the default configuration `current-format-config` gets mutated.

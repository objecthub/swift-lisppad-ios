...
public final class DrawMapLibrary: NativeLibrary {
  ...
  private func makeMapSnapshot(_ center: Expr,
    ...
    var snapshot: MKMapSnapshotter.Snapshot? = nil
    var snapshotError: Error? = nil
    ...
    if let error = snapshotError {
      throw RuntimeError.os(error)
    } else if let sh = snapshot {
      return .object(MapSnapshot(sh))
    } else {
      return .false
    }
  }
